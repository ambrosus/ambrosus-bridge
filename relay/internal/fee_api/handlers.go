...
func (p *FeeAPI) getFees(req reqParams) (*result, *AppError) {
	...
	bridgeFee, err := p.getBridgeFee(bridge, thisCoinPrice, tokenAddress, (*big.Int)(req.Amount))
	if err != nil {
...
func (p *FeeAPI) getTransferFee(bridge networks.BridgeFeeApi, bridgePT BridgePriceTracker, thisCoinPrice, sideCoinPrice float64) (*big.Int, error) {
	...
	transferFeeI, err, _ := p.cache.Memoize("gasPerWithdraw"+bridge.GetName(), func() (interface{}, error) {
	...
	feeSideNative := transferFeeI.(*big.Int)
...
func (p *FeeAPI) getTokenPrice(bridge networks.BridgeFeeApi, tokenAddress common.Address) (float64, error) {
