...
describe("Proxy MultiSig test", () => {
    ...
    it("upgradeTo", async () => {
        ...
        await expect(proxy.connect(adminS).confirmTransaction(0)).to.emit(proxy, "Upgraded")
        ...
        await proxy.submitTransaction(proxy.address, 0, callData);
    ...
    it("MultiSig submitTransaction", async () => {
        ...
        await proxy.submitTransaction(mockERC20.address, 0, callData);
