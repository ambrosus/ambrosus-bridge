...
3. The front-end makes requests to the relay to find out transfer fee and bridge fee
4. On the network from which he wants to withdraw money
...
3.  пока вызовы `withdraw` происходят в одном таймфрейме - трансферы просто добавляются в очередь.\
    ...
    * withdraw\_queue очищается
    * event\_id инкрементируется на 1
...
2. сверяет что `event_id == EthBridge.inputEventId + 1`, иначе ищет Transfer c подходящим event\_id
...
Для экономии газа relay будет подготавливать блоки для смарт контракта таким образом, что бы вместо `rlpEncode` использовать конкатенацию (`abi.encodePacked`).
